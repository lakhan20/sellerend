...
$selectSalesOrder="SELECT * FROM sales_product_details LEFT OUTER JOIN sales_orders ON sales_product_details.sales_orders_idsales_orders=idsales_orders LEFT OUTER JOIN product on product_idproduct=product.idproduct WHERE sales_orders.User_idRegister=$id";
...
if(mysqli_num_rows($res) >0){
    ...
    while($saleOrderRow=mysqli_fetch_assoc($res)){
        ...
        <th scope="row"><button type="button" class="btn btn-success">Accept</button></th>
        <th scope="row"><button type="button" class="btn btn-danger">danger</button></th>
        ';
